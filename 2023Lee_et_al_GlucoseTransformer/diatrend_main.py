...
def train_fold_models(fold_splits, data_dir, model_dir, past_sequence_length, future_offset, batch_size, max_interval_minutes):
    ...
    for fold in fold_splits.keys():
        ...
        save_dir = os.path.join(model_dir, f'saved_models_diatrend/5_fold_sh{past_sequence_length}/')
...
def evaluate_fold_models(fold_splits, data_dir, model_dir, evaluation_dir, past_sequence_length, future_offset, batch_size, max_interval_minutes):
    ...
    for fold in fold_splits.keys():
        ...
        for test in fold_splits[fold]['test']:
            ...
            metrics = evaluate_and_save_metrics_diatrend(
                model=model,
                test_df=test_df,
                save_dir=os.path.join(evaluation_dir, f'evaluation_metrics_diatrend/5_fold_individual_sh{past_sequence_length}/'),
    ...
    df.to_csv(os.path.join(evaluation_dir, f'evaluation_metrics_diatrend/5_fold_test_eval_sh{past_sequence_length}.csv'), index=False)


def main(data_dir):
    """Main function to train and evaluate models for the Diatrend dataset.
    
    Args:
        data_dir (str): Path to the diatrend dataset directory
    """
    # Setup directories
    model_dir, evaluation_dir = setup_directories()

    # Data directory
    # data_dir = '../../dataset/diatrend_subset/'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train and evaluate Diatrend models')
