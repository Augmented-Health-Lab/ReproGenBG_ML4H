...
from cgms_data_seg_diatrend import CGMSDataSeg  # Assuming CGMSDataSeg is defined in cgms_data_seg.py
from AccurateBG.accurate_bg.cnn_ohio import regressor, regressor_transfer, test_ckpt
...
def main(dataset_path, fold_num):
    """
    ...
    # Load training data
    train_directory_path = os.path.join(dataset_path, f'fold{fold_num}_training')
    ...
    # Load test data
    test_directory_path = os.path.join(dataset_path, f'fold{fold_num}_test')
    ...
    for subj in train_file_names:
        subj_path = os.path.join(train_directory_path, f'{subj}.csv')
    ...
    for subj in test_file_names:
        subj_path = os.path.join(test_directory_path, f'{subj}.csv')
        # subj_path = f'../../../datasets/diatrend_subset/fold{fold_num}_test/{subj}.csv'
        test_data[subj] = preprocess_DiaTrend(subj_path)

    # Initialize dataset
    initial_subject_path = os.path.join(dataset_path, 'fold1_training', 'processed_cgm_data_Subject12.csv')
    train_dataset = CGMSDataSeg(
        "diatrend", initial_subject_path, 5
    )
    sampling_horizon = sh
    ...
    for pid in cleaned_test_subjects:
        target_test_dataset_path = os.path.join(test_directory_path, f'processed_cgm_data_{pid}.csv')
        target_test_dataset = CGMSDataSeg(
            "diatrend", target_test_dataset_path, 5
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DiaTrend dataset processing and model training')
    parser.add_argument('--dataset_path', type=str, required=True,
                      help='Base path to the dataset directory')
    parser.add_argument('--fold_num', type=int, required=True, choices=range(1, 6),
                      help='Fold number for cross-validation (1-5)')
    
    args = parser.parse_args()
    main(args.dataset_path, args.fold_num)